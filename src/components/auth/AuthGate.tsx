...
const SignInForm: React.FC<{ onSwitchToSignUp: () => void }> = ({ onSwitchToSignUp }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      ...
      // This exact message comes straight from Supabase Auth itself when
      // email_confirmed_at is genuinely still null for this account - it
      // is never generated by this app. Offer the one thing that
      // actually helps here instead of a dead-end error.
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {!needsConfirmation && <FieldError message={error} />}

      {needsConfirmation && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5 text-sm text-amber-800 space-y-2">
          <p>Your email hasn't been confirmed yet. Check your inbox (and spam folder) for the confirmation link.</p>
          <button
            type="button"
            onClick={handleResend}
            disabled={resendBusy || !email.trim()}
            className="text-amber-900 font-semibold underline underline-offset-2 disabled:opacity-50"
          >
